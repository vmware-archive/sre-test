...
nohup psql -c "select count(*) from public.tab2 a, public.tab3 b where a.generate_series = b.generate_series;" > nohup_spill.out 2>&1 &

sleep 60

echo -e '======================== Check spill ====================\n'

psql -f spill_view.sql

psql -c "select * from spill_vw;"

echo -e '\n\n====================== Connect to sdw1 and kill one primary segment ======================\n'

nohup sh run.sh > tpcd_kill_primary_pid_spill.log 2>&1 &
